...
class ControllerExtensionPaymentPaygol extends Controller 
{
	public function index() 
	{
		...
		$data['merchant_key'] = $this->config->get('paygol_merchant_key');
	...
	public function callback() 
	{
			 ...
			 $secret_key		= 	 $_GET['key'];	
  			...
  			//VALUE
			$secretkey = trim($this->config->get('paygol_merchant_key'));
			$idkey 	   = trim($this->config->get('paygol_merchant_id'));
			
			if ( ($secret_key == $secretkey) && ($service_id == $idkey)) 
			{	
					...
					(isset($_GET['transaction_id']) && !empty($_GET['transaction_id'])) )
				{
					//echo "Callback Ok";
					 ...
					 // UPDATE //
					 $query_update = $this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id='".$completed."' WHERE order_id='".$order_id_form."' AND order_status_id!='".$completed."' ");
					 ...
					 // Update History
					 $query_update_history = $this->db->query("SELECT order_status_id FROM " . DB_PREFIX . "order_history WHERE order_id = '".$order_id."' AND order_status_id='".$completed."'");
					 $history_count = $query_update_history->rows ? count($query_update_history->rows) : 0;
			...
			} else {   echo "<br> Error: Validation error"; exit; }
	}
